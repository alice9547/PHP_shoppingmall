...
<?
	...
	$query = "select * from opts where opt_no70=$row[opt1]";
	$result=mysqli_query($db,$query); 	 
	if (!$result) exit("에러1:$query");                 
	$count1=mysqli_num_rows($result);
	
	$query = "select * from opts where opt_no70=$row[opt2]";
	$result2=mysqli_query($db,$query); 	 
	if (!$result2) exit("에러1:$query");                 
	$count2=mysqli_num_rows($result2);
	
	$query = "select * from opt where no70=$row[opt1]";
	$result3=mysqli_query($db,$query); 	 
	if (!$result3) exit("에러1:$query");      
	$row3=mysqli_fetch_array($result3);
	
	$query = "select * from opt where no70=$row[opt2]";
...
<!-------------------------------------------------------------------------------------------->	
			...
			<input type="hidden" name="no" value="<?=$row[no70];?>">
						...
						<table border="0" cellpadding="0" cellspacing="1" width="315" height="315" bgcolor="D4D0C8">
							<tr>
								<td bgcolor="white" align="center">
									<img src="product/<?=$row[image2];?>" height="315" border="0" align="absmiddle" ONCLICK="Zoomimage('<?=$row[no70];?>')" STYLE="cursor:hand">
								</td>
							</tr>
						</table>
					</td>
						...
						<table border="0" cellpadding="0" cellspacing="0" width="370" class="cmfont">
							...
							<tr>
								...
								<td style="padding-left:10px">
									<font color="282828"><?=$row[name70];?></font><br>
<?
							...
							if($row[icon_sale70]==1) echo("<img src='images/i_sale.gif' align='absmiddle' vspace='1'>");
?>									
								</td>
							</tr>
							<tr><td colspan="3" bgcolor="E8E7EA"></td></tr>
							...
							<tr>
								...
								<td width="1" bgcolor="E8E7EA"></td>
								<td width="289" style="padding-left:10px"><font color="666666"><?=$row[price70];?> 원</font></td>
							</tr>
							<tr><td colspan="3" bgcolor="E8E7EA"></td></tr>
							<!-- 판매가 -->
							<tr>
							...
							<tr>
								...
								<td style="padding-left:10px">
									<select name="opts1" class="cmfont1">
										<option value=""><?=$row3[name70];?></option>
<?
								for($i=1;$i<$count1+1;$i++)
								{
									$row1=mysqli_fetch_array($result);
									
									echo("<option value='$row1[no70]'>$row1[name70]</option>");
								}
?>
									</select> &nbsp;
									
									<select name="opts2" class="cmfont1">
										<option value=""><?=$row4[name70];?></option>
<?
								for($i=1;$i<$count2+1;$i++)
								{
									$row2=mysqli_fetch_array($result2);
									
									echo("<option value='$row2[no70]'>$row2[name70]</option>");
								}
?>
									</select>
								</td>
							...
							<tr>
								...
								<td width="1" bgcolor="E8E7EA"></td>
								<td style="padding-left:10px">
									<input type="text" name="num" value="<?=$num;?>" size="3" maxlength="5" class="cmfont1"> <font color="666666">개</font>
								</td>
							</tr>
							<tr><td colspan="3" bgcolor="E8E7EA"></td></tr>
						</table>
			...
			<table border="0" cellpadding="0" cellspacing="0" width="746">
				<tr>
					<td align="center">
					<img src="images/beasong.png" width="746" border="0">
					</td>
				</tr>
			</table>
			<table border="0" cellpadding="0" cellspacing="0" width="746" class="cmfont">
